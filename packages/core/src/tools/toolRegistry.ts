...
export function createToolRegistry(): Map<MvpToolName, ToolDefinition> {
  ...
  const definitions = [
    ...
    tool(
      "propose_annotation",
      "Propose appending a user clarification to the Annotations section of a verified imported source document. Never rewrite its Document section.",
      z.object({ path: z.string().min(1), body: z.string().min(1), source: proposalSourceSchema }),
      "medium",
    ),
